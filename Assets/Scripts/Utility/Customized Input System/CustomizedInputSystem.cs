    ...
    GameObject GetTopObject(RaycastHit2D[] allHits)
    {
        ...
        if(allHits.Count() > 0)
        {
            for(int i=0;i< allHits.Count(); i++)
            {
                ...
                if (render)
                {
                    if(SortingLayer.GetLayerValueFromID(render.sortingLayerID) > SortingLayer.GetLayerValueFromID(currentSpriteLayer))
                    {
                    ...
                    else if (SortingLayer.GetLayerValueFromID(render.sortingLayerID) == SortingLayer.GetLayerValueFromID(currentSpriteLayer) && render.sortingOrder > currentSpriteOrder)
                    {
                        //Debug.Log(string.Format("layer name: {0}, layer ID {1}", render.sortingLayerName, SortingLayer.GetLayerValueFromID(render.sortingLayerID)));
                        currentSpriteOrder = render.sortingOrder;
                        currentSpriteLayer = render.sortingLayerID;
