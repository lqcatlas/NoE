...
public class LM_006_LightBulb : LevelMasterBase
{
    ...
    void UpdateCells_AllCellsStatusUpdate()
    {
        ...
        if (hasSwitch)
        {
            AudioCentralCtrl.singleton.PlaySFX(lightbulbHub.elecClips.GetClip());
        }
    }
    ...
    //private bool narrative_lv8_2 = false;
    public override void AddtionalUpdate_Theme(Vector2Int coord)
    {
        //in-play narrative
        /*
        if (!narrative_lv8_1 && levelData.levelIndex == 8 && BoardCalculation.CountStatusX(levelData.curBoard, 1) >= 6)
        {
            narrative_lv8_1 = true;
            TryTypeNextPlayLine(0);
        }
        if (!narrative_lv8_2 && levelData.levelIndex == 8 && BoardCalculation.CountStatusX(levelData.curBoard, 1) >= 12)
        {
            narrative_lv8_2 = true;
